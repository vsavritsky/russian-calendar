...
namespace Vsavritsky\RussianCalendar;
...
class RussianCalendar
{
     ...
     * @param int|string|DateTime $date проверяемая дата
     * @param [] $weekends массив с днями недели, которые являются выходными (по умолчанию [0,6] - воскресенье, суббота)
     * @return bool
     */
    public function isWeekend($date, $weekends = [0, 6])
    {
        $ts = $this->convertTimestamp($date);
        $date = date('Y-m-d', $ts);
        $year = date('Y', $ts);
     ...
     * @return bool
     */
    public function isShortWorkingDay($date)
    {
        $ts = $this->convertTimestamp($date);
        $date = date('Y-m-d', $ts);
        $year = date('Y', $ts);
     ...
     * @return bool
     */
    public function isFullWorkingDay($date, $weekends = array(0, 6))
    {
        $ts = $this->convertTimestamp($date);
        $date = date('Y-m-d', $ts);
        $year = date('Y', $ts);
     ...
     *
     * @param int|string|DateTime $date проверяемая дата
     * @param [] $weekends массив с номерами дней недели, которые являются выходными (по умолчанию [0,6] - воскресенье, суббота)
     * @return bool
     */
    public function isWorkingDay($date, $weekends = [0, 6])
    {
        return ($this->isFullWorkingDay($date, $weekends) or $this->isShortWorkingDay($date));
    }
     ...
     */
    public function getNextWorkingDay($date, $weekends = [0, 6], $format = 'Y-m-d')
    {
        $ts = $this->convertTimestamp($date);
        do {
            $ts += 60 * 60 * 24;
     ...
     * @return [] of string
     */
    public function getWeekendDateArray($date, $weekends = [0, 6], $fullArray = true, $format = 'Y-m-d')
    {
        $weekendDates = [];
